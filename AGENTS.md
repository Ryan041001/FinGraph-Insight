...
- `cd backend && uv run uvicorn app.main:app --reload --host 0.0.0.0 --port 8000`: run the API.
- `cd backend && uv run pytest`: run backend contract tests.
- `cd backend && uv add <package>`: add backend runtime deps.
- `cd backend && uv add --dev <package>`: add backend test/dev deps.
...
Commit each completed small feature after verification, so backend setup remains reviewable and easy to roll back.
